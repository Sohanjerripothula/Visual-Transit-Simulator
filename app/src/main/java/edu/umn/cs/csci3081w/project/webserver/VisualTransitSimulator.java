...
public class VisualTransitSimulator {
    ...
    if (!paused) {
      ...
      for (int i = activeVehicles.size() - 1; i >= 0; i--) {
        ...
        if (currVehicle.isTripComplete()) {
          ...
          if (completedTripVehicle.getDV() instanceof Bus) {
            busFactory.returnVehicle(completedTripVehicle);
          } else if (completedTripVehicle.getDV() instanceof Train) {
  ...
  /**
   * Sets a vehicle issue if it doesnt have one.
   * 
   * @param id the id of the vehicle to set the issue too.
   */
  public void setVehicleIssues(int id) {
    for (int i = activeVehicles.size() - 1; i >= 0; i--) {
      if (activeVehicles.get(i).getLine().isIssueExist()) {
        activeVehicles.set(i, new IssueDecorator(activeVehicles.get(i)));
      }
    }
  }
  ...
  public VehicleConcreteSubject getSubject() {
  ...
  public List<Integer> getVehicleStartTimings() {
    return this.vehicleStartTimings;
  }
  ...
  public List<Integer> getTimeSinceLastVehicle() {
    return this.timeSinceLastVehicle;
  }
  ...
  public void setActiveVehicles(List<Vehicle> vehicles) {
