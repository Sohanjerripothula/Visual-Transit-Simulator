...
public class GetVehiclesCommand extends SimulatorCommand {
  ...
  public void execute(WebServerSession session, JsonObject command) {
    ...
    for (int i = 0; i < vehicles.size(); i++) {
      ...
      String vehicleType = "";
      if (currVehicle.getDV() instanceof SmallBus) {
        vehicleType = SmallBus.SMALL_BUS_VEHICLE;
      } else if (currVehicle.getDV() instanceof LargeBus) {
        vehicleType = LargeBus.LARGE_BUS_VEHICLE;
      } else if (currVehicle.getDV() instanceof ElectricTrain) {
        vehicleType = ElectricTrain.ELECTRIC_TRAIN_VEHICLE;
      } else if (currVehicle.getDV() instanceof DieselTrain) {
  ...
  public VisualTransitSimulator getVts() {
    return simulator;
  }
}
